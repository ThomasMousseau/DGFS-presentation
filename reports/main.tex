...
\subsection{Final Thoughts and Future Work}
\begin{frame}{Personal Opinion on this work}
\footnotesize

 This paper addresses a clear problem with path-space samplers, which is that both PIS and DDS required full trajectory signals to learn. By introducing subtrajectory signals from the GFlowNet framework, they were definitely able to reduce the credit assignment problem. Before being able to use any of these path samplers in settings where we only have the unnormalized density function, mainly in chemistry and physics, we will need much better mode coverage since these unnormalized densities are highly multidimensional. The difficult benchmarks have shown that we are not there yet, but there is still a lot of work to do on mode discovery and sample efficiency in order to use these models in real-life applications.
